...
public class AppGenerator implements AppGeneratorInt {
    ...
    public Application getAppByUUID(Application application) throws AppNotFoundExc {

        String UUID = application.getUUID();
        Application getApp = Optional.ofNullable(appManagerInterface.getIdByUUID(UUID)).orElseThrow(() -> new AppNotFoundExc("no such app"));
